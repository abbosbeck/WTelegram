...
internal sealed class ConsoleUi : BackgroundService
{
    ...
    private async Task RunMenuAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            SysConsole.WriteLine("1 – Download media from a chat/channel");
            SysConsole.WriteLine("2 – Search inside a chat & download");
            SysConsole.WriteLine("3 – Download by message link (t.me/…)");
            SysConsole.WriteLine("4 – List recent chats");
            SysConsole.WriteLine("5 – Download active stories from a user/channel");
            SysConsole.WriteLine("6 – Download pinned stories from a user/channel");
            SysConsole.WriteLine("7 – Download video from URL (YouTube, Instagram, TikTok, …)");
            SysConsole.WriteLine("0 – Exit");
            SysConsole.Write("Choice: ");
            ...
            try
            {
                switch (choice)
                {
                    ...
                    case "0": return;
                    default: SysConsole.WriteLine("Unknown option.\n"); break;
                }
            }
    ...
    private async Task DrivePromptsAsync(LoginSession login)
    {
        try
        {
            ...
            {
                SysConsole.WriteLine();
                SysConsole.WriteLine("Telegram sent a verification code. " +
    ...
    private async Task DownloadByLinkFlowAsync(CancellationToken ct)
    {
        ...
        if (item is null)
        {
            SysConsole.WriteLine("That message has no downloadable media.\n");
            return;
        }

        SysConsole.WriteLine($"Found: [{item.Kind}] {item.DisplayName} ({FileHelpers.FormatSize(item.Size)})");
    ...
    private async Task ListChatsFlowAsync(CancellationToken ct)
    {
        var client = await GetOwnerClientAsync(ct);
        SysConsole.WriteLine("\nLoading dialogs…");
        var chats = await _telegram.ListChatsAsync(client, ct);

        SysConsole.WriteLine($"\n{"ID",-15} {"Type",-10} {"Title"}");
        SysConsole.WriteLine(new string('─', 60));
        foreach (var c in chats)
            SysConsole.WriteLine($"{c.Id,-15} {c.Kind,-10} {c.Title}");
        SysConsole.WriteLine();
    }
    ...
    private async Task DownloadFromUrlFlowAsync(CancellationToken ct)
    {
        ...
        {
            SysConsole.WriteLine("No URL provided.\n");
            return;
        }
        ...
        try
        {
            var path = await _webDownloader.DownloadAsync(url, audioOnly, PrintWebProgress, ct);
            SysConsole.WriteLine();
            SysConsole.WriteLine($"Done. Saved to: {path}\n");
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
    ...
    private async Task PromptAndDownloadAsync(Client client, IReadOnlyList<MediaItem> items, CancellationToken ct)
    {
        if (items.Count == 0)
        {
            SysConsole.WriteLine("No matching media found.\n");
            return;
        }
        ...
        for (int i = 0; i < items.Count; i++)
        {
            ...
            SysConsole.WriteLine($"  [{i + 1}] [{it.Kind,-8}] {idLabel}  {it.DisplayName}  {FileHelpers.FormatSize(it.Size)}{dur}");
        }
    ...
    private async Task DownloadListAsync(Client client, IReadOnlyList<MediaItem> selected, CancellationToken ct)
    {
        ...
        int skipped = await _telegram.DownloadManyAsync(client, selected,
            onCompleted: (done, total) => SysConsole.Write($"\r  progress: {done}/{total} completed   "),
            onItemProgress: PrintTelegramProgress,
            ct);

        SysConsole.WriteLine();
        SysConsole.WriteLine($"Done. Downloaded: {selected.Count - skipped}, skipped (already present): {skipped}.\n");
    }
    ...
    private static void PrintTelegramProgress(long transferred, long total, TimeSpan elapsed)
    {
        ...
        SysConsole.Write($"\r  {bar} {pct,5:F1}%  {FileHelpers.FormatSize(transferred)}/{FileHelpers.FormatSize(total)}  {mbps:F2} MB/s  ");
    }

    private static void PrintWebProgress(YoutubeDLSharp.DownloadProgress p)
    {
        ...
        if (pct > 0)
        {
            ...
            SysConsole.Write($"\r  {bar} {pct,5:F1}%  {state,-15}{speed}{eta}   ");
        }
        else
        {
            SysConsole.Write($"\r  {state,-15}{speed}{eta}                                   ");
        }
    }
}
