...
/// <see cref="Client"/> via <c>SessionPool.AcquireAsync</c>.
/// </summary>
public sealed class TelegramService
{
    ...
    public record ChatInfo(long Id, string Kind, string? Title);

    public async Task<IReadOnlyList<ChatInfo>> ListChatsAsync(Client client, CancellationToken ct)
    {
        ...
        foreach (var (id, chat) in dialogs.chats)
        {
            ...
            list.Add(new ChatInfo(id, kind, chat.Title));
        }

        foreach (var (id, user) in dialogs.users)
        {
            if (!user.IsActive) continue;
            list.Add(new ChatInfo(id, "User", $"{user.first_name} {user.last_name}".Trim()));
        }
    ...
    /// <summary>
    /// Downloads many items in parallel. The optional <paramref name="onItemProgress"/>
    /// is invoked with (transferred, total, elapsed) for the currently downloading item
    /// only when running with a degree of parallelism of 1; otherwise it is ignored
    /// (callers should rely on <paramref name="onCompleted"/> for aggregate progress).
    /// </summary>
    public async Task<int> DownloadManyAsync(
        Client client,
        IReadOnlyList<MediaItem> items,
        Action<int, int> onCompleted,
        Action<long, long, TimeSpan>? onItemProgress,
        CancellationToken ct)
    {
        ...
        await Parallel.ForEachAsync(items, opts, async (item, token) =>
        {
            ...
            try
            {
                ...
                await DownloadMediaAsync(client, item, silent ? null : onItemProgress, token);
            }
    ...
    public async Task DownloadMediaAsync(
        Client client,
        MediaItem item,
        Action<long, long, TimeSpan>? onProgress,
        CancellationToken ct)
    {
        ...
        {
            Client.ProgressCallback? cb = onProgress is null
                ? null
                : (transferred, total) => onProgress(transferred, total, sw.Elapsed);
